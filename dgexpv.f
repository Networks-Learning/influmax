*----------------------------------------------------------------------|
      subroutine DGEXPV( n, nz, m, t, v, w, tol, anorm, wsp,
     .                   lwsp, iwsp,liwsp, itrace, a, ia, ja, iflag )

      implicit none
      integer n, nz, m, lwsp, liwsp, itrace, iflag, iwsp(liwsp), ia(nz),
     .                 ja(nz)
...
*     via the external routine `dgcoov' performing the matrix-vector 
...
*
*     dgcoov : external subroutine for matrix-vector multiplication.
*              synopsis: dgcoov( x, y )
...
*
      ...
      do 200 j = 1,m
         nmult = nmult + 1
         call dgcoov( wsp(j1v-n), wsp(j1v), n, nz, ia, ja, a )
         do i = 1,j
